...
const Group = (props) => {
	...
	const group_info = (group, group_info) => {
		...
		if (group !== undefined && group_info !== undefined) {
			...
			battery_pack =
				group_info.battery_pack + "/" + group_info.max_battery_pack;
			...
			if (group_info.role === "tagger") {
				return (
					<div style={{ margin: "0px 0px 5px 0px", textAlign: "center" }}>
						<Badge bg={background_out} text={text}>
							<div className='text-center'>
								<Badge bg={color_theme}>{group.theme}</Badge>
								<Badge bg={color_online}>{color_online}</Badge>
								<Badge bg={color_role}>{group_info.device_name}</Badge>
							...
							<div className='text-center'>
								게임 상태 :{" "}
								<Badge bg={color_game_state} text='dark'>
									{group_info.game_state}
								</Badge>{" "}
								장치 상태 :{" "}
								<Badge bg={color_device_state} text='dark'>
									{group_info.device_state}
								</Badge>
							</div>
							<div className='text-center'>
